...
class AddCompetitionView extends StatelessWidget {
  const AddCompetitionView({super.key, required publishDate});
    ...
    return BlocBuilder<AddCompetitionBloc, AddCompetitionState>(
      builder: (context, state) {
        if (state is AddCompetitionSignUpLoadingState) {
          ...
          return FormWidget(context, addCompetitionId: state.addCompetitionId);
        } else {
          return FormWidget(context, addCompetitionId: '');
        }
      },
    );
  }
